...
if ($_POST)
{
    ...
    $conn = mysqli_connect("localhost","root","94d548bb171","researchdb");
     $myFile = "research.xml";
    ...
    $rss_txt .= '<research>';
    $query = "SELECT s.id, g.grade,s.guid,s.datevisited FROM `session` as s join grade as g on s.`gradeid` = g.id order by s.`datevisited` desc ;";
    ...
    while($values_query = $result->fetch_assoc())
    {
        ...
        while($values_query2 = $result2->fetch_assoc())
        {
            ...
            while($values_query3 = $result3->fetch_assoc())
            {
                ...
                $rss_txt .="<position>".$values_query3['position']."</position>";
